        ...
        <div className='space-y-2 mt-4 '>
            {
                notificationData.map((item) => (
                    <div key={item.id} className='flex justify-between items-center px-5 py-3 rounded-lg bg-white'>
                        <div>
                            <h4 className='font-Bold text-lg text-black'>{item.title}</h4>
                            <p className='font-PoppinsRegular text-base text-black'>{item.description}</p>
                        </div>
                        <div>
                            <p className='text-sm font-bold text-black'>{item.date.getHours()} hours ago</p>
                            <p className='text-sm font-PoppinsMedium text-black'>{item.date.toLocaleDateString()}</p>
                        </div>
                    </div>
                ))
            }
